...
# Từ danh sách year_months, lấy khoảng ngày bao trùm từ tháng sớm nhất đến tháng muộn nhất.
...
# Validate payload trả về từ Open-Meteo. Trả về số dòng daily.
def _validate_weather_payload(payload: dict[str, Any], expected_daily_variables: list[str]) -> int:
    ...
    for field in expected_daily_variables:
        ...
        if len(values) != row_count:
            raise ValueError(f"Weather API field '{field}' length mismatch: expected {row_count}, got {len(values)}")
